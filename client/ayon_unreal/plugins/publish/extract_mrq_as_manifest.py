...
class ExtractMRQAsManifest(publish.Extractor):
    ...
    def serialize_mrq(self, instance):
        ...
        _, manifest = unreal.MoviePipelineEditorLibrary.save_queue_to_manifest_file(
            self.mrq
        )
        manifest_string = (
            unreal.MoviePipelineEditorLibrary.convert_manifest_file_to_string(manifest)
        )
    ...
    def copy_manifest_to_publish(self, instance):
        # NOTE: we could also add the manifest to p4
        #       might run into issues tho bc the manifest
        #       gets extracted after workspace validation
