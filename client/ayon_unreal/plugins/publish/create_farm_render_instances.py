...
class CreateFarmRenderInstances(publish.AbstractCollectRender):
    ...
    def get_instances(self, context):
        ...
        if not unreal.EditorAssetLibrary.does_asset_exist(render_queue_path):
        ...
        for inst in context:
            ...
            if not job and not auto_handle_mrq:
                ...
                sequence_uasset = unreal.EditorAssetLibrary.load_asset(
                    inst.data["sequence"]
                )
                ...
                if render_preset:
                    asset_filter = unreal.ARFilter(
                        class_names=["MoviePipelinePrimaryConfig"], recursive_paths=True
                    )
